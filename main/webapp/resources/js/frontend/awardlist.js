$(function() {
	...
	var pageSize = 10;
	//获取奖品列表的url
	var listUrl = '/gradudes/frontend/listawardsbyshop';
	//兑换奖品的url
	var exchangeUrl = '/gradudes/frontend/adduserawardmap';
	var pageNum = 1;
	//从地址栏获取shopId
			...
			if (data.success) {
				...
				data.awardList.map(function(item, index) {
					html += '' + '<div class="card" data-award-id="'
							...
							+ getContextPath() + item.awardImg + '"width="44">'
	...
	$('.award-list')
			.on(
					...
					'.card',
					function(e) {
						//若用户在该店铺有积分并且积分数大于该奖品积分
						if(canProceed&&(totalPoint>=e.currentTarget.dataset.point)){
							//则弹出操作确认框
							$.confirm(
							  '需要消耗的积分'+e.currentTarget.dataset.point
							  +'积分，确定操作么',
							  function(){
								  //访问后台，领取奖品
								  $.ajax({
