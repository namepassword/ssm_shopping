...
import com.cxxy.qiu.gradudes.dao.ProductImgDao;
...
public class ProductServiceImpl implements ProductService{
	...
	public ProductExecution addProduct(Product product, ImageHolder thumbnail,
		...
		if(product!=null&&product.getShop()!=null&&product.getShop().getShopId()!=null){
			...
			try{
				...
				throw new ProductOperationException("创建商品是被："+e.getMessage());
			}
			//若商品详情图不为空则添加
	...
	private void addProductImgList(Product product,
			List<ImageHolder> productImgHolderList) throws ProductOperationException {
		// 获取图片存储路径，这里直接放到相应店铺的文件夹下
		String dest=PathUtil.getShopImagePath(product.getShop().getShopId());
		List<ProductImg> productImgList= new ArrayList<ProductImg>();
		//遍历图片一次处理，并添加进ProductImg实体类
	...
	public ProductExecution modifyProduct(Product product,
		...
		if(product!=null&&product.getShop()!=null&&product.getShop().getShopId()!=null){
			//给商品设置上默认属性
			product.setLastEditTime(new Date());
			//若商品缩略图不为空且原图缩略图不为空则删除原有缩略图并添加
			
			
			if(thumbnail!=null){
				//先获取一遍原有信息，因为原来的信息里有缘图片地址
	...
	public ProductExecution getProductList(Product productCondition,
		...
		//页码转换成数据库的行码，并调用dao层获取指定页码的商品列表
